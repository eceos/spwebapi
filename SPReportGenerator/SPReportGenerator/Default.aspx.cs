...
namespace SPReportGenerator
{
    ...
    public partial class _Default : Page
    {
        ...
        {
            var list = getList("Tasks");
            string toDisplay = string.Empty;

            foreach (var item in list)
            {
                string boldStart = item["StartDate"].Equals(item["DueDate"]) ? "<strong>" : string.Empty;
                string boldEnd = item["StartDate"].Equals(item["DueDate"]) ? "<strong>" : string.Empty;

                toDisplay += string.Format("Task Name {0}{1}{2}, Start Date {3}, End Date {4}\n", boldStart, 
                    item["Title"], boldEnd, item["StartDate"], item["DueDate"]);
            }

            Label1.Text = toDisplay;
        }
    }
}
